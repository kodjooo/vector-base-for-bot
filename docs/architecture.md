...
- `RAGCorpusLoader` (`app/load_rag_corpus.py`) — CLI-загрузчик структурированного `rag_chunks.jsonl` в ChromaDB; использует поле `embedding_text` для эмбеддингов, сохраняет `answer` как возвращаемый текст и переносит диагностические метаданные.
...
- Рекомендуемый режим наполнения базы поддержки — структурированный RAG-корпус `artifacts/rag_corpus/rag_chunks.jsonl`: каждый JSONL-объект является самостоятельным смысловым блоком с `doc_id`, `doc_title`, `section_path`, `block_type`, `question_intents`, `keywords`, `answer`, `source_facts`, `embedding_text` и `metadata`.
- `app/load_rag_corpus.py` полностью пересоздаёт коллекцию ChromaDB и загружает JSONL-корпус. Для эмбеддинга используется расширенное поле `embedding_text` с названием документа, путём раздела, пользовательскими интентами, ключевыми словами и ответом; в качестве документа Chroma сохраняется поле `answer`, чтобы бот получал компактный готовый контекст без служебного шума.
- Для базы поддержки используются компактные чанки по умолчанию (`EMBEDDING_CHUNK_SIZE=350`, `EMBEDDING_CHUNK_OVERLAP=60`) и смысловой чанкинг в `SyncOrchestrator`: текст сначала делится по заголовочным маркерам и границам предложений, затем собирается в чанки без начала с середины фразы. Это уменьшает смешивание разных разделов интерфейса и точнее отвечает на короткие пользовательские вопросы.
- `VectorStoreGateway` (`app/chroma_manager.py`) использует `chromadb.HttpClient` для доступа к коллекции (по умолчанию `knowledge`), удаляет устаревшие записи документа, добавляет новые чанки с метаданными и умеет полностью заменить коллекцию готовым RAG-корпусом.
- Поиск релевантных фрагментов выполняется через `VectorStoreGateway.search`: сервис запрашивает расширенный набор semantic-кандидатов из ChromaDB, дополнительно просматривает документы коллекции для keyword-кандидатов, объединяет результаты и выполняет универсальное intent-aware переранжирование.
- Переранжирование учитывает совпадения терминов в тексте и метаданных, покрытие запроса, тип запроса (`navigation`, `instruction`, `reconciliation`, `general`), наличие UI-навигационных маркеров (`раздел`, `вкладка`, `кнопка`, `колонка`, `строка`, `фильтр`) и действий (`нажать`, `открыть`, `выбрать`, `раскрыть`). Для запросов сверки дополнительно учитываются маркеры расхождения, задержек, отчётов и выплат. В коде не используются правила под конкретный бизнес-кейс или конкретный термин.
- После ранжирования результат с текстовыми совпадениями сжимается до сфокусированного excerpt вокруг релевантных предложений. Это снижает шум от соседних абзацев и уменьшает риск, что бот ответит по нерелевантной части большого чанка.
- В метаданные чанков при синхронизации добавляются ориентировочные поля `section`, `title`, `preview`; они используются для диагностики поиска, логирования в support-bot и дополнительного keyword-rerank.
- HTTP API `/search` сохраняет обратную совместимость через поле `chunks`, но дополнительно возвращает `results` со структурой `text`, `metadata`, `distance`, `score`, `semantic_score`, `keyword_score`, `matched_terms`.
- Параметры `SEARCH_TOP_K`, `SEARCH_CANDIDATE_MULTIPLIER`, `SEARCH_MIN_SCORE` и `SEARCH_KEYWORD_LIMIT` управляют числом итоговых результатов, числом semantic-кандидатов, минимальным итоговым баллом и глубиной keyword-поиска.
