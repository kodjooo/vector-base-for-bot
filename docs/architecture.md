...
- Переранжирование учитывает совпадения терминов в тексте и метаданных, покрытие запроса, тип запроса (`navigation`, `instruction`, `general`), наличие UI-навигационных маркеров (`раздел`, `вкладка`, `кнопка`, `колонка`, `строка`, `фильтр`) и действий (`нажать`, `открыть`, `выбрать`, `раскрыть`). В коде не используются правила под конкретный бизнес-кейс или конкретный термин.
