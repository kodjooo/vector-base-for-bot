...
- Для базы поддержки используются компактные чанки по умолчанию (`EMBEDDING_CHUNK_SIZE=350`, `EMBEDDING_CHUNK_OVERLAP=60`), чтобы один результат поиска реже смешивал разные разделы интерфейса и точнее отвечал на короткие пользовательские вопросы.
...
- Поиск релевантных фрагментов выполняется через `VectorStoreGateway.search`: сервис запрашивает расширенный набор semantic-кандидатов из ChromaDB, дополнительно просматривает документы коллекции для keyword-кандидатов, объединяет результаты и выполняет переранжирование с boost по точным совпадениям терминов пользовательского запроса.
